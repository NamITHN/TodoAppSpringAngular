...
export class TodoService {
  ...
  private dataurl: string = "http://localhost:8080/todo";
  ...
  postdata(newinfo: FormAddData): Observable<FormAddData> {
    return this.http.post<FormAddData>(this.dataurl, newinfo, httpOptions).pipe(
      tap(data => {
  ...
  deleteselect(listtodo: number[]): Observable<any> {
    ...
    const link: string = `${this.dataurl}/multiple?listId=${list
  ...
  updatedata(
    newdata: FormAddData,
    danhsachid: number
  ): Observable<FormAddData> {
    const urlput = `${this.dataurl}/${danhsachid}`;
    console.log(urlput);
    return this.http.put<FormAddData>(urlput, newdata, httpOptions).pipe(
      tap(data => {
